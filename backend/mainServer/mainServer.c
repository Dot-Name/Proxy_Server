...
#ifdef AS_CLIENT
  snprintf(cmd, sizeof(cmd), "ip addr add 10.8.0.2/16 dev tun0");
  run(cmd);
  snprintf(cmd, sizeof(cmd), "ip link set tun0 mtu %d up", MTU);
#else
  snprintf(cmd, sizeof(cmd), "ip addr add 10.8.0.1/16 dev tun0");
  run(cmd);
  snprintf(cmd, sizeof(cmd), "ip link set tun0 mtu %d up", MTU);
#endif
  run(cmd);
...
#ifdef AS_CLIENT
  system("iptables -t nat -D POSTROUTING -o tun0 -j MASQUERADE 2>/dev/null");
  system("iptables -D FORWARD -i tun0 -m state --state RELATED,ESTABLISHED -j ACCEPT 2>/dev/null");
  system("iptables -D FORWARD -o tun0 -j ACCEPT 2>/dev/null");
  char cmd[1024];
  snprintf(cmd, sizeof(cmd), "ip route del %s 2>/dev/null", SERVER_HOST);
  system(cmd);
  system("ip route del 0/1 2>/dev/null");
  system("ip route del 128/1 2>/dev/null");
#else
  system("iptables -t nat -D POSTROUTING -s 10.8.0.0/16 ! -d 10.8.0.0/16 -m comment --comment 'vpndemo' -j MASQUERADE 2>/dev/null");
  system("iptables -D FORWARD -s 10.8.0.0/16 -m state --state RELATED,ESTABLISHED -j ACCEPT 2>/dev/null");
  system("iptables -D FORWARD -d 10.8.0.0/16 -j ACCEPT 2>/dev/null");
#endif
}
...
int udp_bind(struct sockaddr *addr, socklen_t* addrlen) {
  ...
  if (0 != getaddrinfo(host, port_str, &hints, &result)) {
